...
/**
 * Validate an uploaded document before we spend any CPU rasterizing it.
 *
...
export async function validateDocument(
  ...
  // Anything else (a real non-PDF, or a non-PDF wearing a .pdf extension).
  const detail = detected
    ? `Detected ${detected.mime} (.${detected.ext}).`
    : "Could not recognise the file as a PDF.";
  throw new ExtractionError(
